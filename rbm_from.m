...
filename = sprintf('matlab_epoch%d.mat', from_epoch-1);
load(filename);

fprintf('start weights:\n');
vishid(1:5,1:5)
...
for epoch = from_epoch:maxepoch
 ...
 %for batch = 1:599
   ...
   %%%%%%%%% UPDATE WEIGHTS AND BIASES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    if true
        filename = sprintf('matlab_from%d_epoch%d-%05d.mat', from_epoch, epoch, ...
            batch*batchsize);
  ...
  filename = sprintf('matlab_from%d_epoch%d.mat', from_epoch, epoch);
  save(filename, 'vishid', 'visbiases', 'hidbiases');
