...
%  ylabel('Variance of log weights','fontsize',14)
 ...
 %%% Initialize biases of the base rate model by ML %%%%%%%%%%%%%%%%%%%%%%%
   ...
   save('basebiases.mat', 'visbiases_base');
 else
 ...
 negdata = negdata > rand(numcases,numdims); % equals base_sample_vis (base visible samples)
 logww  =  logww - (negdata*visbiases_base' + numhids*log(2)); % substracts log p_0*(v_0)

 Wh = negdata*vishid + hidbias;      % hidden activations (without beta factor)
 ...
 for bb = beta(2:end-1);  
   ...
   expWh = exp(bb*Wh);
   logww  =  logww + (1-bb)*Bv_base + bb*Bv + sum(log(1+expWh),2); % adds log p_i*(v_(i-1))
   
   poshidprobs = expWh./(1 + expWh);     % hidden probabilities (with beta factor)
   poshidstates = poshidprobs > rand(numcases,numhids);  % hidden samples

   negdata = 1./(1 + exp(-(1-bb)*visbias_base - bb*(poshidstates*vishid' + visbias))); % visible probabilities (with beta factor)
   negdata = negdata > rand(numcases,numdims);  % visible samples
...
%    end 
   ...
   expWh = exp(bb*Wh);
   logww  =  logww - ((1-bb)*Bv_base + bb*Bv + sum(log(1+expWh),2)); % substracts log p_i*(v_i)
   
 end 

 expWh = exp(Wh);
 logww  = logww +  negdata*visbiases' + sum(log(1+expWh),2);  % adds log p_N*(v_(N-1))
 ...
 %fprintf('base log Z: %f', logZZ_base);
